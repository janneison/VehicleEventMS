...
from app.core.domain.entities import (
    ...
    RutaEspecialDetalle, PuntoControlEspecial, RutaEspecialControl, EventoResumen
)
from app.infrastructure.adapters.database.models import (
    Vehiculos, Eventos, EventosDesc, Procesos, EjesViales, PeriodosActivo,
    PeriodosConductores, ProgramacionVehicularModel, Odométros, Recursos, EventosResumen,
    ProgEspecialesVehiculos, RutasEspecialesDetalles, PuntosControl, RutasEspecialesControl
)
...
def _to_periodo_conductor_entity(model: PeriodosConductores) -> Optional[PeriodoConductor]:
    if not model:
        return None
...
def _to_programacion_especial_vehiculo_entity(model: ProgEspecialesVehiculos) -> Optional[ProgramacionEspecialVehiculo]:
    if not model:
        return None
...
def _to_ruta_especial_detalle_entity(model: RutasEspecialesDetalles) -> Optional[RutaEspecialDetalle]:
    if not model:
        return None
...
class VehicleEventRepositoryImpl(VehicleEventRepository):
    ...
    async def save_event(self, event: VehicleEvent) -> int:
        new_event = Eventos(
            ...
            evento=event.event_code,
            fecha=event.processed_date,
            velocidad=event.processed_speed,
    ...
    async def get_last_event_with_gps(self, vehicle_id: str) -> Optional[VehicleEvent]:
        ...
        stmt = select(Eventos).where(
            Eventos.idvehiculo == vehicle_id,
            Eventos.latitud != 'null', # Assuming 'null' as string for invalid GPS
            Eventos.longitud != 'null',
            Eventos.latitud != 'N', # Handle malformed string like 'N' which can become 0.0
            Eventos.longitud != 'W',
            Eventos.fecha > datetime.now() - timedelta(days=2) # SP uses 'now() - 2 days'
        ).order_by(Eventos.idevento.desc()).limit(1) # Order by idevento_ desc in SP
        ...
        if event_model:
            return VehicleEvent(
                ...
                event_code=event_model.evento,
                system_date_str="", # Not available
                speed=event_model.velocidad,
                latitude_raw=event_model.latitud,
                longitude_raw=event_model.longitud,
                ip_address="", port=0, # Not available
                geofence_index=event_model.indicegeocerca,
                vehicle_on=False, signal_status="", realtime_date=event_model.fecha,
                address=event_model.direccion, city=event_model.municipio, department=event_model.departamento,
    ...
    async def insert_ejes_viales(self, address: str, city: str, latitude: float, longitude: float, department: str):
        ...
        new_entry = EjesViales(
            ...
            flat_geom=func.ST_Transform(func.ST_GeomFromText(f'POINT({longitude} {latitude})', 4326), 21892),
            xpos=0,
            ypos=0
        )
    ...
    async def find_evento_descripcion(self, event_code: int) -> Optional[EventoDescripcion]:
        stmt = select(EventosDesc).where(EventosDesc.evento == str(event_code)) # event is text in DB
        result = await self.session.execute(stmt)
...
class VehicleRepositoryImpl(VehicleRepository):
    ...
    async def get_active_vehicle_by_id(self, vehicle_id: str) -> Optional[Vehicle]:
        stmt = select(Vehiculos).where(Vehiculos.idvehiculo == vehicle_id, Vehiculos.estado == 'Y')
        result = await self.session.execute(stmt)
    ...
    async def update_vehicle_status(self, vehicle: Vehicle):
        ...
        if vehicle_orm:
            ...
            vehicle_orm.idconductor_actual = vehicle.idconductor_actual # Can be set to NULL by SP logic
    ...
    async def update_resource_gps_status(self, recurso_id: str, contratista_id: str, event_date: datetime, gps_ok: bool):
        stmt = select(Recursos).where(Recursos.recurso == recurso_id, Recursos.contratista == contratista_id)
        result = await self.session.execute(stmt)
        recurso_orm = result.scalar_one_or_none()
        if recurso_orm:
            recurso_orm.fechagps = event_date
            recurso_orm.estadogps = 'OK' if gps_ok else 'NOTOK' # SP uses 'okgps' variable
            await self.session.flush()
...
class PeriodRepositoryImpl(PeriodRepository):
    ...
    async def get_last_periodo_conductor_for_reset(self, vehicle_id: str, driver_id: int, current_date: datetime) -> Optional[PeriodoConductor]:
        ...
        ).order_by(PeriodosConductores.fechadesde.desc()).limit(1) # Get the last one
        ...
        if last_period and last_period.fechahasta:
            if (current_date - last_period.fechahasta) < timedelta(minutes=1):
                return last_period
        elif last_period and last_period.fechahasta is None: # If an active period exists
            return last_period
        return None


    async def update_periodo_conductor_end_date(self, period_id: int, end_date: Optional[datetime]):
        stmt = select(PeriodosConductores).where(PeriodosConductores.idperiodo == period_id)
        result = await self.session.execute(stmt)
        periodo_orm = result.scalar_one_or_none()
        if periodo_orm:
            periodo_orm.fechahasta = end_date
    ...
    async def deactivate_current_driver(self, vehicle_id: str, driver_id: int):
        stmt = select(Vehiculos).where(Vehiculos.idvehiculo == vehicle_id, Vehiculos.idconductor_actual == driver_id)
        result = await self.session.execute(stmt)
        vehicle_orm = result.scalar_one_or_none()
        if vehicle_orm:
...
class SpecialRouteRepositoryImpl(SpecialRouteRepository):
    ...
    async def get_nearby_special_route_detail(self, route_id: int, latitude: float, longitude: float) -> Optional[RutaEspecialDetalle]:
        ...
        stmt = (
            ...
            .join(PuntosControl, RutasEspecialesDetalles.idpunto == PuntosControl.idpunto)
            .outerjoin(RutasEspecialesControl, and_(
                RutasEspecialesControl.idpunto == RutasEspecialesDetalles.idpunto,
                RutasEspecialesControl.idprogramacion == ProgEspecialesVehiculos.idprogramacion # This join needs to be careful
            ))
            .where(
                RutasEspecialesDetalles.idruta == route_id,
                func.ST_Distance(
                    func.ST_Transform(current_point, 2163), # Transform to a projected CRS for distance in meters
                    func.ST_Transform(func.ST_GeomFromText(text(f"'POINT(' || puntoscontrol.longitud || ' ' || puntoscontrol.latitud || ')'"), 4326), 2163)
                ) <= PuntosControl.radio,
    ...
    async def get_initial_tiempoglobal(self, route_id: int) -> Optional[float]:
        # SP query: SELECT tiempoglobal FROM rutas_especiales_detalles WHERE idruta = progespecial_.idruta ORDER BY orden limit 1;
        stmt = select(RutasEspecialesDetalles.tiempoglobal).where(RutasEspecialesDetalles.idruta == route_id).order_by(RutasEspecialesDetalles.orden).limit(1)
        result = await self.session.execute(stmt)
