...
class Eventos(Base):
    ...
    evento = Column(Integer) # Renamed from 'evento' to 'event_code' in domain but maps to 'evento' in DB
...
class EjesViales(Base):
    ...
    flat_geom = Column(ga.Geometry('POINT', srid=21892)) # Assumed SRID 21892 for ST_Transform from SP
    xpos = Column(Float)
    ypos = Column(Float)
...
class PeriodosConductores(Base):
    ...
    idperiodo = Column(BigInteger, primary_key=True, autoincrement=True) # Assuming a primary key
    idvehiculo = Column(String)
    idconductor = Column(BigInteger)
    fechadesde = Column(DateTime)
    fechahasta = Column(DateTime)

class ProgramacionVehicularModel(Base): # To check active programs (for idconductor_actual reset)
...
class Recursos(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True) # Assuming primary key
    recurso = Column(String)
    contratista = Column(String)
